# --- INICIO main.py v2.2.2 (PostgreSQL + Docs v1 - SyntaxError Corregido) ---
...
def extraer_texto_pdf_docx(ruta_archivo: str, extension: str) -> str:
    ...
    try:
        ...
        else: return "" # Solo procesa estos tipos
                            ...
                            if file_ext in ['pdf', 'doc', 'docx', 'txt', 'csv']:
                                ...
                                try:
                                    ...
                                    if file_ext in ['pdf', 'doc', 'docx']: extracted_text = extraer_texto_pdf_docx(temp_path, file_ext)
                                         ...
                                         except Exception as read_err: logging.error(f"Error leyendo archivo texto {temp_path}: {read_err}")
                                finally:
                                    try: os.remove(temp_path); logging.info(f"Archivo temporal eliminado: {temp_path}")
    ...
    try:
        if content_type in IMAGE_MIMES:
            ...
            try:
                ...
                logging.info(f"Archivo guardado temporalmente en: {ruta_temporal}") # Log añadido
                ...
                # No necesitamos un 'else' aquí, si no se extrajo texto, la siguiente validación fallará
            
            finally:
                # Bloque finally CORREGIDO para borrar el archivo temporal
            ...
            # Validar si se pudo extraer texto
            ...
            logging.info(f"Texto extraído OK (longitud: {len(texto_extraido)}). Preparando payload.")
